...
static void sinkhorn_variance_normalize(
    ...
    const bool log_std_enabled =
        !env_enabled("LLAMA_KVARN_DISABLE_LOG_STD_SINKHORN") &&
...
static void test_pending_k_layout_store(uint32_t head_dim, bool paper_frame) {
    ...
    try {
        const std::vector<int32_t> two_records = { 0, 1 };
        ggml_cuda_kvarn_store_body_pending_records_minmax(
                pending_k_d, pending_v_d,
                k_body_d, v_body_d,
                k_scales_d, v_scales_d,
    ...
    require(rejected_multi_record_pending,
            "CUDA pending-record store rejects unsafe multi-record batches");
...
static void test_pending_heads_store(uint32_t head_dim) {
    ...
    const size_t n = size_t(head_dim)*group;
    const size_t pending_token_stride = size_t(head_dim)*n_heads;
    const llama_kvarn_layout layout = llama_kvarn_make_layout(params, head_dim);

    std::vector<float> pending_k(size_t(group)*pending_token_stride);
    std::vector<float> pending_v(pending_k.size());
    for (uint32_t g = 0; g < group; ++g) {
        for (uint32_t ih = 0; ih < n_heads; ++ih) {
            for (uint32_t d = 0; d < head_dim; ++d) {
                const size_t off = size_t(g)*pending_token_stride + size_t(ih)*head_dim + d;
                pending_k[off] =
                    0.013f*std::sin(float(d + 17*g + 101*ih)*0.021f) +
                    0.019f*std::cos(float(3*d + 5*g + 29*ih)*0.017f);
                pending_v[off] =
                    0.011f*std::cos(float(7*d + 11*g + 37*ih)*0.019f) -
                    0.023f*std::sin(float(d + 13*g + 41*ih)*0.013f);
            }
        }
    }

    std::vector<uint8_t> k_body_ref(size_t(n_heads)*layout.k_body_bytes);
    std::vector<uint8_t> v_body_ref(size_t(n_heads)*layout.v_body_bytes);
    std::vector<float> k_scales_ref(size_t(n_heads)*layout.k_scale_floats);
    std::vector<float> v_scales_ref(size_t(n_heads)*layout.v_scale_floats);
    for (uint32_t ih = 0; ih < n_heads; ++ih) {
        std::vector<float> k_rec(n);
        std::vector<float> v_rec(n);
        for (uint32_t g = 0; g < group; ++g) {
            for (uint32_t d = 0; d < head_dim; ++d) {
                const size_t src = size_t(g)*pending_token_stride + size_t(ih)*head_dim + d;
                k_rec[size_t(d)*group + g] = pending_k[src];
        ...
        std::copy(rec.v_scales.begin(), rec.v_scales.end(), v_scales_ref.begin() + size_t(ih)*layout.v_scale_floats);
    }
    ...
    require_cuda(cudaMalloc(&v_body_d, v_body_ref.size()), "cudaMalloc pending-heads V body");
    require_cuda(cudaMalloc(&k_scales_d, k_scales_ref.size()*sizeof(float)), "cudaMalloc pending-heads K scales");
    require_cuda(cudaMalloc(&v_scales_d, v_scales_ref.size()*sizeof(float)), "cudaMalloc pending-heads V scales");
    require_cuda(cudaMalloc(&scratch_d, scratch_floats*sizeof(float)), "cudaMalloc pending-heads scratch");
    ...
    require_cuda(cudaMemcpy(k_body.data(), k_body_d, k_body.size(), cudaMemcpyDeviceToHost), "copy pending-heads K body");
    require_cuda(cudaMemcpy(v_body.data(), v_body_d, v_body.size(), cudaMemcpyDeviceToHost), "copy pending-heads V body");
    require_cuda(cudaMemcpy(k_scales.data(), k_scales_d, k_scales.size()*sizeof(float), cudaMemcpyDeviceToHost), "copy pending-heads K scales");
    require_cuda(cudaMemcpy(v_scales.data(), v_scales_d, v_scales.size()*sizeof(float), cudaMemcpyDeviceToHost), "copy pending-heads V scales");
    ...
    if (packed_diff > 4 || scale_err >= 1.0e-5f) {
        std::fprintf(stderr,
                "pending-heads store diff: head_dim=%u packed_diff=%zu scale_err=%g\n",
                head_dim, packed_diff, double(scale_err));
    }
    require(packed_diff <= 4, "CUDA pending-heads store packed bytes match independent reference");
    require(scale_err < 1.0e-5f, "CUDA pending-heads store scales match independent reference");
...
static void run_case(uint32_t head_dim, float rtn_quantile) {
    ...
    const char * scale_max_kind = "";
    size_t scale_max_i = 0;
    float scale_max_ref = 0.0f;
    float scale_max_cuda = 0.0f;
    for (size_t i = 0; i < record.k_scales.size(); ++i) {
        const float err = std::fabs(record.k_scales[i] - k_scales_store[i]);
        if (err > scale_max_err) {
            scale_max_err = err;
            scale_max_kind = "K";
            scale_max_i = i;
            scale_max_ref = record.k_scales[i];
            scale_max_cuda = k_scales_store[i];
        }
    }
    for (size_t i = 0; i < record.v_scales.size(); ++i) {
        const float err = std::fabs(record.v_scales[i] - v_scales_store[i]);
        if (err > scale_max_err) {
            scale_max_err = err;
            scale_max_kind = "V";
            scale_max_i = i;
            scale_max_ref = record.v_scales[i];
            scale_max_cuda = v_scales_store[i];
        }
    }
    const bool log_std_enabled =
        !env_enabled("LLAMA_KVARN_DISABLE_LOG_STD_SINKHORN") &&
        (std::getenv("LLAMA_KVARN_ENABLE_LOG_STD_SINKHORN") == nullptr ||
         env_enabled("LLAMA_KVARN_ENABLE_LOG_STD_SINKHORN"));
    if (!log_std_enabled && scale_max_err >= 1.0e-5f) {
        std::fprintf(stderr,
                "store scale diff: head_dim=%u rtn=%.2f max_err=%g kind=%s index=%zu ref=%g cuda=%g\n",
                head_dim, params.rtn_quantile, double(scale_max_err),
                scale_max_kind, scale_max_i, double(scale_max_ref), double(scale_max_cuda));
    }
    if (!log_std_enabled) {
    ...
    if (head_dim == 256) {
        ...
        auto run_q36_runtime_warpqk_shape = [&](uint32_t n_rt_queries, uint32_t n_rt_records, uint32_t n_rt_pending,
                uint32_t n_rt_tail, uint32_t rt_tail_start, const char * label) {
            const uint32_t n_rt_tokens = n_q36_sink + n_rt_records*group + n_rt_pending + n_rt_tail;
            const uint32_t rt_mask_stride_tokens = 512;

            std::vector<float> rt_queries(size_t(n_rt_queries)*n_head*head_dim);
            for (uint32_t iq = 0; iq < n_rt_queries; ++iq) {
                for (uint32_t ih = 0; ih < n_head; ++ih) {
                    for (uint32_t d = 0; d < head_dim; ++d) {
                        rt_queries[(size_t(iq)*n_head + ih)*head_dim + d] =
                            0.117f*std::sin(float(d + 3*ih + 19*iq)*0.021f) -
                            0.069f*std::cos(float(2*d + 11*ih + 7*iq)*0.015f);
                    }
                }
            }
            ...
            std::vector<float> rt_pending(size_t(std::max<uint32_t>(1, n_rt_pending))*n_head_kv*head_dim, 0.0f);
            for (uint32_t t = 0; t < n_rt_pending; ++t) {
                for (uint32_t ikh = 0; ikh < n_head_kv; ++ikh) {
                    for (uint32_t d = 0; d < head_dim; ++d) {
                        rt_pending[(size_t(t)*n_head_kv + ikh)*head_dim + d] =
                            0.061f*std::sin(float(d + 13*t + 17*ikh)*0.023f) -
                            0.047f*std::cos(float(3*d + 5*t + 11*ikh)*0.017f);
                    }
                }
            }

            float * rt_queries_d = cuda_upload(rt_queries);
            float * rt_mask_d = cuda_upload(rt_mask);
            float * rt_pending_d = cuda_upload(rt_pending);
            float * rt_split_d = nullptr;
            float * rt_fused_d = nullptr;
            float * rt_scores_d = nullptr;
            float * rt_fused_scores_d = nullptr;
            require_cuda(cudaMalloc(&rt_split_d, rt_queries.size()*sizeof(float)), "cudaMalloc runtime split output");
            require_cuda(cudaMalloc(&rt_fused_d, rt_queries.size()*sizeof(float)), "cudaMalloc runtime fused output");
            require_cuda(cudaMalloc(&rt_scores_d, n_rt_tokens*sizeof(float)), "cudaMalloc runtime split scores");
            require_cuda(cudaMalloc(&rt_fused_scores_d, n_rt_tokens*sizeof(float)), "cudaMalloc runtime fused scores");

            set_env_var("LLAMA_KVARN_ATTN_SPLIT_KERNELS", "1");
            ggml_cuda_kvarn_attn_mixed_f16_batch(
                    rt_queries_d, q36_sink_tail_k_d, q36_sink_tail_v_d,
                    mha_k_body_d, mha_v_body_d, mha_k_scales_d, mha_v_scales_d,
                    rt_pending_d, rt_pending_d, rt_mask_d,
                    rt_split_d, rt_scores_d,
                    n_rt_queries, n_head, n_head_kv,
                    n_q36_sink, n_rt_records, n_rt_pending, n_rt_tail, rt_tail_start, head_dim, group,
                    params.key_bits, params.value_bits,
                    head_dim, size_t(n_head)*head_dim,
                    head_dim, size_t(n_head)*head_dim,
                    head_dim, size_t(n_head_kv)*head_dim,
                    ...
                    records[0].k_scales.size(), records[0].v_scales.size(),
                    size_t(n_records)*records[0].k_scales.size(), size_t(n_records)*records[0].v_scales.size(),
                    size_t(rt_mask_stride_tokens)*sizeof(float), sizeof(float), 1,
                    scale,
                    nullptr);
            set_env_var("LLAMA_KVARN_ATTN_SPLIT_KERNELS", "");
            require_cuda(cudaGetLastError(), "KVarN CUDA runtime-shape split launch");
            require_cuda(cudaDeviceSynchronize(), "KVarN CUDA runtime-shape split sync");
            ...
            ggml_cuda_kvarn_attn_mixed_f16_batch(
                    ...
                    nullptr);
            set_env_var("LLAMA_KVARN_ATTN_FUSED_BATCH", "");
            require_cuda(cudaGetLastError(), "KVarN CUDA runtime-shape forced fused launch");
            require_cuda(cudaDeviceSynchronize(), "KVarN CUDA runtime-shape forced fused sync");

            std::vector<float> rt_split(rt_queries.size());
            std::vector<float> rt_fused(rt_queries.size());
            require_cuda(cudaMemcpy(rt_split.data(), rt_split_d, rt_split.size()*sizeof(float), cudaMemcpyDeviceToHost),
                    "copy runtime-shape split output");
            ...
            cudaFree(rt_mask_d);
            cudaFree(rt_pending_d);
            cudaFree(rt_split_d);
            cudaFree(rt_fused_d);
            cudaFree(rt_scores_d);
            cudaFree(rt_fused_scores_d);
        };

        run_q36_runtime_warpqk_shape(384, 1, 0, 128, 0, "q384-rec1-tail128");
        run_q36_runtime_warpqk_shape(1, 1, 1, 128, 1, "q1-rec1-pending1-tail128");
    ...
    if (head_dim == 512) {
...

        {
            ...
            std::vector<float> gemma_rt_sink_tail_k_ref(gemma_rt_sink_tail_k.size());
            std::vector<float> gemma_rt_sink_tail_v_ref(gemma_rt_sink_tail_v.size());
            for (uint32_t t = 0; t < n_gemma_rt_sink + n_gemma_rt_tail; ++t) {
                for (uint32_t d = 0; d < head_dim; ++d) {
                    const size_t off = size_t(t)*head_dim + d;
                    const float k = 0.087f*std::sin(float(d + 13*t)*0.017f) -
                                    0.051f*std::cos(float(3*d + 19*t)*0.011f);
                    const float v = 0.079f*std::cos(float(5*d + 17*t)*0.013f) +
                                    0.047f*std::sin(float(d + 29*t)*0.019f);
                    gemma_rt_sink_tail_k[off] = f32_to_f16_bits(k);
                    gemma_rt_sink_tail_v[off] = f32_to_f16_bits(v);
                    gemma_rt_sink_tail_k_ref[off] = f16_bits_to_f32(gemma_rt_sink_tail_k[off]);
                    gemma_rt_sink_tail_v_ref[off] = f16_bits_to_f32(gemma_rt_sink_tail_v[off]);
                }
            }

            std::vector<float> gemma_rt_queries_v(size_t(gemma_rt_queries)*gemma_rt_n_head*head_dim);
            for (uint32_t iq = 0; iq < gemma_rt_queries; ++iq) {
                for (uint32_t ih = 0; ih < gemma_rt_n_head; ++ih) {
                    for (uint32_t d = 0; d < head_dim; ++d) {
                        gemma_rt_queries_v[(size_t(iq)*gemma_rt_n_head + ih)*head_dim + d] =
                            0.151f*std::sin(float(d + 7*ih + 31*iq)*0.023f) -
                            0.097f*std::cos(float(2*d + 5*ih + 13*iq)*0.017f);
                    }
                }
            }

            std::vector<float> gemma_rt_pending_k(size_t(n_gemma_rt_pending)*head_dim);
            std::vector<float> gemma_rt_pending_v(gemma_rt_pending_k.size());
            for (uint32_t t = 0; t < n_gemma_rt_pending; ++t) {
                for (uint32_t d = 0; d < head_dim; ++d) {
                    const size_t off = size_t(t)*head_dim + d;
                    gemma_rt_pending_k[off] =
                        0.063f*std::sin(float(d + 11*t)*0.029f) -
                        0.041f*std::cos(float(3*d + 17*t)*0.021f);
                    gemma_rt_pending_v[off] =
                        0.057f*std::cos(float(5*d + 13*t)*0.027f) +
                        0.039f*std::sin(float(d + 19*t)*0.031f);
                }
            }

            std::vector<float> gemma_rt_mask(size_t(gemma_rt_queries)*gemma_rt_mask_stride_tokens, -1.0e30f);
            for (uint32_t iq = 0; iq < gemma_rt_queries; ++iq) {
                for (uint32_t t = 0; t < gemma_rt_mask_stride_tokens; ++t) {
                    gemma_rt_mask[size_t(iq)*gemma_rt_mask_stride_tokens + t] =
                        t <= n_gemma_rt_sink + n_gemma_rt_records*group + iq ? 0.0f : -1.0e30f;
                }
            }

            float * gemma_rt_queries_d = cuda_upload(gemma_rt_queries_v);
            uint16_t * gemma_rt_sink_tail_k_d = cuda_upload(gemma_rt_sink_tail_k);
            uint16_t * gemma_rt_sink_tail_v_d = cuda_upload(gemma_rt_sink_tail_v);
            float * gemma_rt_pending_k_d = cuda_upload(gemma_rt_pending_k);
            float * gemma_rt_pending_v_d = cuda_upload(gemma_rt_pending_v);
            ...
            ggml_cuda_kvarn_attn_mixed_f16_batch(
                    ...
                    gemma_rt_split_d, gemma_rt_scores_d,
                    ...
                    nullptr);
            set_env_var("LLAMA_KVARN_ATTN_SPLIT_KERNELS", "");
            require_cuda(cudaGetLastError(), "KVarN CUDA Gemma runtime-shape split launch");
            require_cuda(cudaDeviceSynchronize(), "KVarN CUDA Gemma runtime-shape split sync");
            ...
            ggml_cuda_kvarn_attn_mixed_f16_batch(
                    ...
                    multi_k_body_d, multi_v_body_d, multi_k_scales_d, multi_v_scales_d,
                    gemma_rt_pending_k_d, gemma_rt_pending_v_d, gemma_rt_mask_d,
                    gemma_rt_fused_d, gemma_rt_fused_scores_d,
                    gemma_rt_queries, gemma_rt_n_head, gemma_rt_n_head_kv,
                    n_gemma_rt_sink, n_gemma_rt_records, n_gemma_rt_pending, n_gemma_rt_tail, gemma_rt_tail_start,
                    head_dim, group, params.key_bits, params.value_bits,
                    head_dim, size_t(gemma_rt_n_head)*head_dim,
            ...
            cudaFree(gemma_rt_sink_tail_v_d);
            cudaFree(gemma_rt_pending_k_d);
            cudaFree(gemma_rt_pending_v_d);
