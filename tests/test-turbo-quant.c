...
extern void quantize_row_tq6_0_ref(const float * x, void * y, long long k);
extern void dequantize_row_tq6_0(const void * x, float * y, long long k);
extern void quantize_row_tq5_0_ref(const float * x, void * y, long long k);
extern void dequantize_row_tq5_0(const void * x, float * y, long long k);
...
int main(void) {
    ...
    for (int i = 0; i < d; i++) input[i] = cosf(i*0.2f) * 5.0f;
    quantize_row_tq6_0_ref(input, buf, d);
    dequantize_row_tq6_0(buf, output, d);
    turbo_cpu_fwht_inverse(output, d);
    printf("Test 3b (tq6): cos*5\n");
    printf("  In:  [%.4f, %.4f, %.4f, %.4f]\n",  (double)(input[0]), (double)(input[1]), (double)(input[2]), (double)(input[3]));
    printf("  Out: [%.4f, %.4f, %.4f, %.4f]\n",  (double)(output[0]), (double)(output[1]), (double)(output[2]), (double)(output[3]));
    mse = cosv = ni = no = 0;
    for (int i = 0; i < d; i++) { mse += (input[i]-output[i])*(input[i]-output[i]); cosv += input[i]*output[i]; ni += input[i]*input[i]; no += output[i]*output[i]; }
    ...
    printf("  MSE=%.8f RMSE=%.6f sigma Cosine=%.6f\n",  (double)(mse/d), (double)(sqrtf(mse/ni)), (double)(cosv/sqrtf(ni)/sqrtf(no)));
    {
        const float cos_tq6  = cosv/sqrtf(ni)/sqrtf(no);
        const float rmse_tq6 = sqrtf(mse/ni);
        if (!(cos_tq6 > 0.999f) || !(rmse_tq6 < 0.04f)) {
            printf("=== FAILED: tq6 cosine %.6f rmse %.6f sigma (want > 0.999, < 0.04) ===\n",
                   (double) cos_tq6, (double) rmse_tq6);
            return 1;
        }
    }
    printf("\n");
     ...
     * (tq6 0.025, turbo4 0.097). Dequant leaves the output in the rotated domain, so apply
     * the inverse WHT before comparing. */
    for (int i = 0; i < d; i++) input[i] = cosf(i*0.2f) * 5.0f;
    quantize_row_tq5_0_ref(input, buf, d);
    dequantize_row_tq5_0(buf, output, d);
    turbo_cpu_fwht_inverse(output, d);
    printf("Test 3c (tq5): cos*5\n");
    printf("  In:  [%.4f, %.4f, %.4f, %.4f]\n",  (double)(input[0]), (double)(input[1]), (double)(input[2]), (double)(input[3]));
    printf("  Out: [%.4f, %.4f, %.4f, %.4f]\n",  (double)(output[0]), (double)(output[1]), (double)(output[2]), (double)(output[3]));
    mse = cosv = ni = no = 0;
    for (int i = 0; i < d; i++) { mse += (input[i]-output[i])*(input[i]-output[i]); cosv += input[i]*output[i]; ni += input[i]*input[i]; no += output[i]*output[i]; }
    ...
    printf("  MSE=%.8f RMSE=%.6f sigma Cosine=%.6f\n",  (double)(mse/d), (double)(sqrtf(mse/ni)), (double)(cosv/sqrtf(ni)/sqrtf(no)));
    {
        const float cos_tq5  = cosv/sqrtf(ni)/sqrtf(no);
        const float rmse_tq5 = sqrtf(mse/ni);
        if (!(cos_tq5 > 0.998f) || !(rmse_tq5 < 0.07f)) {   // 5-bit Lloyd-Max: rmse ~0.05 sigma -> cos ~ 1 - rmse^2/2 = 0.99875
            printf("=== FAILED: tq5 cosine %.6f rmse %.6f sigma (want > 0.998, < 0.07) ===\n",
                   (double) cos_tq5, (double) rmse_tq5);
            return 1;
        }
    }
    printf("\n");
    ...
    printf("Test 4: chunked dequant == whole-row dequant\n");
    {
        const enum ggml_type types[] = {
            ...
            GGML_TYPE_TQ6_0, GGML_TYPE_TQ5_0,
        };
        const char * names[] = { "tq3_1s", "tq4_1s", "turbo2_0", "turbo3_0", "turbo4_0", "tq6_0", "tq5_0" };
