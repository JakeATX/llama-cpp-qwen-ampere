...
int main() {
    ...
    t.test("CUDA backend computes block-safe KV page geometry", [](testing & t) {
        ...
        // head_dim_v == 0 is invalid input, not a "K-only, no V region"
        // signal - see the comment above ggml_cuda_kv_stream_page_bytes()
        // in fattn.cu for why that special-casing was tried and reverted.
        t.assert_true("head_dim_v == 0 is rejected", !page_bytes_fn(
            GGML_TYPE_Q8_0, GGML_TYPE_Q8_0, 256, 0, 4, 256, &page_bytes));
    });
