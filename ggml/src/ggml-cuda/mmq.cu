...
void ggml_cuda_mul_mat_q_pair(
    ...
    GGML_ASSERT(mmq_get_q8_1_ds_layout(src0_a->type) == mmq_get_q8_1_ds_layout(src0_b->type));
    ...
    GGML_ASSERT(ggml_backend_buffer_get_usage(src0_a->buffer) != GGML_BACKEND_BUFFER_USAGE_COMPUTE);
    GGML_ASSERT(ggml_backend_buffer_get_usage(src0_b->buffer) != GGML_BACKEND_BUFFER_USAGE_COMPUTE);
    ...
    const int64_t ne02 = src0_a->ne[2];
    const int64_t ne03 = src0_a->ne[3];
    ...
    GGML_ASSERT(src0_a->nb[0] == ggml_type_size(src0_a->type));
    GGML_ASSERT(src1->nb[0] == sizeof(float));
    GGML_ASSERT(dst_a->nb[0] == sizeof(float));

    const int64_t ne10_padded = GGML_PAD(ne10, MATRIX_ROW_PADDING);
    const bool fallback = ne01 % 128 != 0;
    const int64_t j_max = std::max(ggml_cuda_mmq_get_J_max(src0_a->type, fallback, cc, ne11),
                                   ggml_cuda_mmq_get_J_max(src0_b->type, fallback, cc, ne11));
    ...
    quantize_mmq_q8_1_cuda((const float *) src1->data, nullptr, src1_q8_1.get(), src0_a->type,
    ...
    const auto launch = [&](const ggml_tensor * src0, ggml_tensor * dst) {
        ...
        const mmq_args args = {
            ...
            nullptr, nullptr, (float *) dst->data, nullptr,
            ne00, ne01, dst->ne[1], s01, ne11, s1,
            ne02, ne12, s02, s12_q, s2,
            ne03, ne13, s03, s13_q, s3,
            dst->ne[1]};
        ggml_cuda_mul_mat_q_switch_type(ctx, args, stream);
    };
