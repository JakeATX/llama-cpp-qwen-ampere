...
enum {
    GGML_CUDA_KVARN_V_LAYOUT_LEGACY          = 0,
    GGML_CUDA_KVARN_V_LAYOUT_TURBO_CANONICAL = 1,
    GGML_CUDA_KVARN_V_LAYOUT_LEGACY_R1       = 2,
    GGML_CUDA_KVARN_V_LAYOUT_LEGACY_R2       = 3,
    GGML_CUDA_KVARN_V_LAYOUT_LEGACY_R3       = 4,
    GGML_CUDA_KVARN_V_LAYOUT_SPARSE_D512     = 5,
};

static int64_t ggml_cuda_kvarn_v_body_bytes(int32_t head_dim, int32_t group_size, int32_t value_bits, int32_t v_layout) {
    const int64_t n = int64_t(head_dim)*group_size;
    if (v_layout == GGML_CUDA_KVARN_V_LAYOUT_LEGACY) {
        return (n*value_bits + 7)/8;
    }
    if (v_layout == GGML_CUDA_KVARN_V_LAYOUT_LEGACY_R1 ||
            v_layout == GGML_CUDA_KVARN_V_LAYOUT_LEGACY_R2 ||
            v_layout == GGML_CUDA_KVARN_V_LAYOUT_LEGACY_R3) {
        if (group_size != 128 || (head_dim != 256 && head_dim != 512) || value_bits != 2) {
            return -1;
        }
        const int64_t rank = v_layout == GGML_CUDA_KVARN_V_LAYOUT_LEGACY_R3 ? 3 :
            (v_layout == GGML_CUDA_KVARN_V_LAYOUT_LEGACY_R2 ? 2 : 1);
        return (n*value_bits + 7)/8 + rank*int64_t(sizeof(uint16_t))*(group_size + head_dim);
    }
    if (v_layout == GGML_CUDA_KVARN_V_LAYOUT_SPARSE_D512) {
        return head_dim == 512 && group_size == 128 && value_bits == 2 ? 21504 : -1;
    }
    if (v_layout != GGML_CUDA_KVARN_V_LAYOUT_TURBO_CANONICAL) {
        return -1;
    }
    if (group_size != 128 || (head_dim % 128) != 0 || (value_bits != 2 && value_bits != 4)) {
        return -1;
    }
...
static bool ggml_cuda_kvarn_v_body_shape_ok(int64_t actual_bytes, int64_t expected_bytes, int32_t v_layout) {
    if (expected_bytes < 0) {
        return false;
    }
    return v_layout == GGML_CUDA_KVARN_V_LAYOUT_LEGACY ?
        actual_bytes >= expected_bytes : actual_bytes == expected_bytes;
}

static uint32_t ggml_cuda_kvarn_v_mode(int32_t v_layout) {
    return v_layout == GGML_CUDA_KVARN_V_LAYOUT_TURBO_CANONICAL ? 2u :
        (v_layout == GGML_CUDA_KVARN_V_LAYOUT_LEGACY_R1 ? 3u :
         (v_layout == GGML_CUDA_KVARN_V_LAYOUT_LEGACY_R2 ? 4u :
          (v_layout == GGML_CUDA_KVARN_V_LAYOUT_LEGACY_R3 ? 5u :
           (v_layout == GGML_CUDA_KVARN_V_LAYOUT_SPARSE_D512 ? 6u : 0u))));
}

static bool ggml_cuda_kvarn_v_layout_is_residual(int32_t v_layout) {
    return v_layout == GGML_CUDA_KVARN_V_LAYOUT_LEGACY_R1 ||
        v_layout == GGML_CUDA_KVARN_V_LAYOUT_LEGACY_R2 ||
        v_layout == GGML_CUDA_KVARN_V_LAYOUT_LEGACY_R3 ||
        v_layout == GGML_CUDA_KVARN_V_LAYOUT_SPARSE_D512;
}
...
static bool ggml_cuda_kvarn_timing_enabled() {
    return ggml_cuda_kvarn_env_flag("LLAMA_KVARN_CUDA_TIMING");
}
...
static bool ggml_cuda_kvarn_timing_claim() {
    static std::atomic<int> n_trace{0};

    const int limit = ggml_cuda_kvarn_env_int("LLAMA_KVARN_CUDA_TIMING_LIMIT", 256);
    if (limit <= 0) {
        return true;
    }
...
class ggml_cuda_kvarn_timing_scope {
public:
    ggml_cuda_kvarn_timing_scope(cudaStream_t stream, std::string label)
        : stream(stream), label(std::move(label)) {
        active = ggml_cuda_kvarn_timing_enabled() && ggml_cuda_kvarn_timing_claim();
        if (!active) {
            return;
        }
    ...
    ~ggml_cuda_kvarn_timing_scope() {
        ...
        std::fprintf(stderr, "KVarN CUDA timing: %s elapsed_ms=%.6f\n", label.c_str(), double(ms));
...
static std::atomic<uint64_t> g_ggml_cuda_kvarn_boundary_dump_count{0};

static int ggml_cuda_kvarn_env_nonneg_int(const char * name, int default_value) {
    const char * env = std::getenv(name);
    if (env == nullptr) {
        return default_value;
    }
    ...
    if (env[0] == '\0' || end == nullptr || *end != '\0' || errno == ERANGE || value < 0 ||
        ...
        GGML_ABORT("invalid KVarN CUDA environment integer %s=%s; expected non-negative integer", name, env);
    }
    return int(value);
}
...
static int ggml_cuda_kvarn_parse_uint_after_marker(const char * name, const char * marker) {
    if (name == nullptr || marker == nullptr || marker[0] == '\0') {
        return -1;
    }
    ...
    char * parse_end = nullptr;
    errno = 0;
    const long value = std::strtol(s.c_str() + start, &parse_end, 10);
    if (parse_end == nullptr || errno == ERANGE ||
            value < 0 || value > std::numeric_limits<int>::max()) {
        return -1;
    }
    return int(value);
...
static void ggml_cuda_kvarn_write_binary_file(const std::filesystem::path & path, const void * data, size_t nbytes) {
    ...
    if (!f) {
        GGML_ABORT("failed to open KVarN boundary dump file %s", path.string().c_str());
    }
    f.write(reinterpret_cast<const char *>(data), std::streamsize(nbytes));
    if (!f) {
        GGML_ABORT("failed to write KVarN boundary dump file %s", path.string().c_str());
    }
}

static void ggml_cuda_kvarn_copy_strided_rows(
        std::vector<uint8_t> & dst,
        const uint8_t * base,
        size_t row_bytes,
        size_t row_stride_bytes,
        uint32_t n_rows) {
    dst.assign(size_t(n_rows)*row_bytes, uint8_t(0));
    for (uint32_t r = 0; r < n_rows; ++r) {
        CUDA_CHECK(cudaMemcpy(dst.data() + size_t(r)*row_bytes, base + size_t(r)*row_stride_bytes, row_bytes, cudaMemcpyDeviceToHost));
    }
}
...
static void ggml_cuda_kvarn_maybe_dump_boundary(
        const ggml_tensor * q,
        const ggml_tensor * q_body,
        const ggml_tensor * sink_tail_k,
        const ggml_tensor * sink_tail_v,
        const ggml_tensor * body_k,
        const ggml_tensor * body_v,
        const ggml_tensor * scales_k,
        const ggml_tensor * scales_v,
        const ggml_tensor * pending_k,
        const ggml_tensor * pending_v,
    ...
    const char * dir_env = std::getenv("LLAMA_KVARN_ATTN_BOUNDARY_DUMP_DIR");
    const std::filesystem::path root = dir_env != nullptr && dir_env[0] != '\0' ?
        std::filesystem::path(dir_env) : std::filesystem::path("artifacts/kvarn-boundary/default");
    std::ostringstream stem;
    stem << "call_" << std::setw(6) << std::setfill('0') << call_index;
    std::filesystem::path dir = root / stem.str();
    for (int suffix = 1; std::filesystem::exists(dir); ++suffix) {
        std::ostringstream alt;
        alt << stem.str() << "_" << suffix;
        dir = root / alt.str();
    }
    std::filesystem::create_directories(dir);

    const uint32_t n_queries = uint32_t(out->ne[2]);
    ...
    const bool body_mirror_allowed = head_dim >= 512 ||
        (head_dim == 256 && !ggml_cuda_kvarn_env_flag("LLAMA_KVARN_ATTN_DISABLE_BODY_F32_MIRROR"));
    ...
    const uint8_t * q_row = reinterpret_cast<const uint8_t *>(q->data) + size_t(selected_iq)*q->nb[2] + size_t(selected_ih)*q->nb[1];
    ggml_cuda_kvarn_copy_strided_rows(bytes, q_row, size_t(head_dim)*sizeof(float), q->nb[0], 1);
    ggml_cuda_kvarn_write_binary_file(dir / "q.bin", bytes.data(), bytes.size());
    const bool has_q_body = q_body != nullptr;
    if (has_q_body) {
        const uint8_t * q_body_row = reinterpret_cast<const uint8_t *>(q_body->data) + size_t(selected_iq)*q_body->nb[2] + size_t(selected_ih)*q_body->nb[1];
        ggml_cuda_kvarn_copy_strided_rows(bytes, q_body_row, size_t(head_dim)*sizeof(float), q_body->nb[0], 1);
        ggml_cuda_kvarn_write_binary_file(dir / "q_body.bin", bytes.data(), bytes.size());
    }

    const uint8_t * out_row = reinterpret_cast<const uint8_t *>(out->data) + size_t(selected_iq)*out->nb[2] + size_t(selected_ih)*out->nb[1];
    ...
    if (dump_full_qo) {
        ...
        if (q->nb[0] != sizeof(float) ||
                ...
                q->nb[2] != size_t(head_dim)*size_t(n_head)*sizeof(float) ||
                out->nb[0] != sizeof(float) ||
                out->nb[1] != size_t(head_dim)*sizeof(float) ||
                out->nb[2] != size_t(head_dim)*size_t(n_head)*sizeof(float)) {
            GGML_ABORT("LLAMA_KVARN_ATTN_BOUNDARY_DUMP_FULL_QO requires contiguous q/out tensors");
        }
        bytes.assign(full_qo_bytes, uint8_t(0));
        CUDA_CHECK(cudaMemcpy(bytes.data(), q->data, full_qo_bytes, cudaMemcpyDeviceToHost));
        ggml_cuda_kvarn_write_binary_file(dir / "full_q.bin", bytes.data(), bytes.size());
        if (has_q_body) {
            if (q_body->nb[0] != sizeof(float) ||
                    q_body->nb[1] != size_t(head_dim)*sizeof(float) ||
                    q_body->nb[2] != size_t(head_dim)*size_t(n_head)*sizeof(float)) {
                GGML_ABORT("LLAMA_KVARN_ATTN_BOUNDARY_DUMP_FULL_QO requires contiguous q_body tensor");
            }
            CUDA_CHECK(cudaMemcpy(bytes.data(), q_body->data, full_qo_bytes, cudaMemcpyDeviceToHost));
            ggml_cuda_kvarn_write_binary_file(dir / "full_q_body.bin", bytes.data(), bytes.size());
        }
    ...
    ggml_cuda_kvarn_write_binary_file(dir / "body_k.bin", bytes.data(), bytes.size());
    ggml_cuda_kvarn_copy_strided_rows(bytes, reinterpret_cast<const uint8_t *>(body_v->data) + size_t(selected_ikh)*body_v->nb[2], size_t(body_v->ne[0]), body_v->nb[1], uint32_t(n_records));
    ggml_cuda_kvarn_write_binary_file(dir / "body_v.bin", bytes.data(), bytes.size());
    ggml_cuda_kvarn_copy_strided_rows(bytes, reinterpret_cast<const uint8_t *>(scales_k->data) + size_t(selected_ikh)*scales_k->nb[2], size_t(scales_k->ne[0])*sizeof(float), scales_k->nb[1], uint32_t(n_records));
    ggml_cuda_kvarn_write_binary_file(dir / "scales_k.bin", bytes.data(), bytes.size());
    ggml_cuda_kvarn_copy_strided_rows(bytes, reinterpret_cast<const uint8_t *>(scales_v->data) + size_t(selected_ikh)*scales_v->nb[2], size_t(scales_v->ne[0])*sizeof(float), scales_v->nb[1], uint32_t(n_records));
    ggml_cuda_kvarn_write_binary_file(dir / "scales_v.bin", bytes.data(), bytes.size());
    ggml_cuda_kvarn_copy_strided_rows(bytes, reinterpret_cast<const uint8_t *>(pending_k->data) + size_t(selected_ikh)*pending_k->nb[1], size_t(head_dim)*sizeof(float), pending_k->nb[2], uint32_t(n_pending));
    ggml_cuda_kvarn_write_binary_file(dir / "pending_k.bin", bytes.data(), bytes.size());
    ggml_cuda_kvarn_copy_strided_rows(bytes, reinterpret_cast<const uint8_t *>(pending_v->data) + size_t(selected_ikh)*pending_v->nb[1], size_t(head_dim)*sizeof(float), pending_v->nb[2], uint32_t(n_pending));
    ggml_cuda_kvarn_write_binary_file(dir / "pending_v.bin", bytes.data(), bytes.size());

    const size_t f16_row_bytes = size_t(head_dim)*sizeof(uint16_t);
    std::vector<uint8_t> sink_tail_bytes(size_t(uint32_t(n_sink) + uint32_t(n_tail))*f16_row_bytes, uint8_t(0));
    for (uint32_t t = 0; t < uint32_t(n_sink); ++t) {
        const uint8_t * src = reinterpret_cast<const uint8_t *>(sink_tail_k->data) + size_t(selected_ikh)*sink_tail_k->nb[1] + size_t(t)*sink_tail_k->nb[2];
        CUDA_CHECK(cudaMemcpy(sink_tail_bytes.data() + size_t(t)*f16_row_bytes, src, f16_row_bytes, cudaMemcpyDeviceToHost));
    }
    for (uint32_t t = 0; t < uint32_t(n_tail); ++t) {
        const uint32_t tail_slot = uint32_t(n_tail) == 0 ? 0 : (uint32_t(tail_start) + t)%uint32_t(n_tail);
        const uint8_t * src = reinterpret_cast<const uint8_t *>(sink_tail_k->data) + size_t(selected_ikh)*sink_tail_k->nb[1] + size_t(uint32_t(n_sink) + tail_slot)*sink_tail_k->nb[2];
        CUDA_CHECK(cudaMemcpy(sink_tail_bytes.data() + size_t(uint32_t(n_sink) + t)*f16_row_bytes, src, f16_row_bytes, cudaMemcpyDeviceToHost));
    }
    ggml_cuda_kvarn_write_binary_file(dir / "sink_tail_k_f16.bin", sink_tail_bytes.data(), sink_tail_bytes.size());
    for (uint32_t t = 0; t < uint32_t(n_sink); ++t) {
        const uint8_t * src = reinterpret_cast<const uint8_t *>(sink_tail_v->data) + size_t(selected_ikh)*sink_tail_v->nb[1] + size_t(t)*sink_tail_v->nb[2];
        CUDA_CHECK(cudaMemcpy(sink_tail_bytes.data() + size_t(t)*f16_row_bytes, src, f16_row_bytes, cudaMemcpyDeviceToHost));
    }
    for (uint32_t t = 0; t < uint32_t(n_tail); ++t) {
        const uint32_t tail_slot = uint32_t(n_tail) == 0 ? 0 : (uint32_t(tail_start) + t)%uint32_t(n_tail);
        const uint8_t * src = reinterpret_cast<const uint8_t *>(sink_tail_v->data) + size_t(selected_ikh)*sink_tail_v->nb[1] + size_t(uint32_t(n_sink) + tail_slot)*sink_tail_v->nb[2];
        CUDA_CHECK(cudaMemcpy(sink_tail_bytes.data() + size_t(uint32_t(n_sink) + t)*f16_row_bytes, src, f16_row_bytes, cudaMemcpyDeviceToHost));
    }
    ...
    json << "  \"n_sink\": " << n_sink << ",\n";
    json << "  \"n_records\": " << n_records << ",\n";
    json << "  \"n_pending\": " << n_pending << ",\n";
    json << "  \"n_tail\": " << n_tail << ",\n";
    json << "  \"tail_start\": " << tail_start << ",\n";
    ...
    json << "  \"has_q_body\": " << (has_q_body ? "true" : "false") << ",\n";
    json << "  \"q_body_stride_head_floats\": " << (has_q_body ? q_body->nb[1]/sizeof(float) : 0) << ",\n";
    json << "  \"q_body_stride_query_floats\": " << (has_q_body ? q_body->nb[2]/sizeof(float) : 0) << ",\n";
    ...
    std::fprintf(stderr,
            "KVarN boundary dump: call=%" PRIu64 " iq=%u ih=%u ikh=%u tokens=%u records=%d path=%s\n",
            call_index, selected_iq, selected_ih, selected_ikh, n_tokens, n_records, dir.string().c_str());
}
...
static void ggml_cuda_mul_mat(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    if (hint == GGML_HINT_SRC0_IS_HADAMARD) {
        const bool fwht_taken = !split && ggml_cuda_op_fwht(ctx, src1, dst);
        if (ggml_cuda_kvarn_env_flag("LLAMA_KVARN_FWHT_TRACE")) {
            std::fprintf(stderr,
                    "KVarN CUDA FWHT trace: taken=%d split=%d src1_type=%s dst_type=%s"
                    " src1_contig=%d dst_contig=%d src1_ne=[%" PRId64 ",%" PRId64 ",%" PRId64 ",%" PRId64 "]"
                    " dst_ne=[%" PRId64 ",%" PRId64 ",%" PRId64 ",%" PRId64 "] src0=%s src1=%s dst=%s\n",
                    fwht_taken ? 1 : 0, split ? 1 : 0,
                    ggml_type_name(src1->type), ggml_type_name(dst->type),
                    ggml_is_contiguous(src1) ? 1 : 0, ggml_is_contiguous(dst) ? 1 : 0,
                    src1->ne[0], src1->ne[1], src1->ne[2], src1->ne[3],
...
static bool ggml_cuda_compute_forward(ggml_backend_cuda_context & ctx, struct ggml_tensor * dst) {
    switch (dst->op) {
        ...
        case GGML_OP_KVARN_STORE_BODY:
            {
                ...
                const int64_t body_bytes = params.is_v ?
                    ggml_cuda_kvarn_v_body_bytes(params.head_dim, params.group_size, params.bits, params.v_layout) :
                    (n_values*params.bits + 7)/8;
                ...
                std::ostringstream timing_label;
                timing_label << "store-body kind=" << (params.is_v ? 'v' : 'k')
                    << " head_dim=" << params.head_dim
                    << " group_size=" << params.group_size
                    << " bits=" << params.bits
                    << " iters=" << params.sinkhorn_iters;
                ...
                if (params.is_v) {
                    ggml_cuda_kvarn_store_v_body_reference_minmax(
                            ...
                            ggml_cuda_kvarn_v_mode(params.v_layout),
                            params.src_layout == 2,
                            ctx.stream());
                } else {
        ...
        case GGML_OP_KVARN_STORE_KV_BODY:
            {
                ...
                const int64_t k_body_records_cap_store = k_body_root ? k_body_root->ne[1] : (k_body ? k_body->ne[1] : dst->ne[1]);
                ...
                const int64_t v_body_bytes = ggml_cuda_kvarn_v_body_bytes(params.head_dim, params.group_size, params.value_bits, params.v_layout);
                ...
                if (ggml_cuda_kvarn_v_layout_is_residual(params.v_layout) &&
                        (v_body->ne[0] != v_body_bytes ||
                         v_body->nb[0] != 1 ||
                         v_body->nb[1] < size_t(v_body_bytes) ||
                         v_body->nb[2] < v_body->nb[1]*size_t(v_body->ne[1]) ||
                         params.group_size != 128 || params.key_bits != 8 || params.value_bits != 2 ||
                         (params.head_dim != 256 && params.head_dim != 512))) {
                    return false;
                }
                const uint32_t turbo_v_mode = ggml_cuda_kvarn_v_mode(params.v_layout);
                ...
                std::ostringstream timing_label;
                timing_label << "store-kv-body head_dim=" << params.head_dim
                    << " group_size=" << params.group_size
                    << " iters=" << params.sinkhorn_iters
                ...
                if (params.n_record_batch > 0 && params.src_layout == 1) {
                    ...
                    const int debug_record_from_name =
                        ggml_cuda_kvarn_parse_uint_after_marker(k_body ? k_body->name : nullptr, "_r");
                    const int debug_head_from_name =
                        ggml_cuda_kvarn_parse_uint_after_marker(k_body ? k_body->name : nullptr, "_h");
                    const uint32_t debug_record =
                        debug_record_from_name >= 0 ? uint32_t(debug_record_from_name) : UINT32_MAX;
                    const uint32_t debug_head =
                        debug_head_from_name >= 0 ? uint32_t(debug_head_from_name) : UINT32_MAX;
                    ggml_cuda_kvarn_debug_set_store_context(
        ...
        case GGML_OP_KVARN_MATERIALIZE_KV:
            {
                ...
                const ggml_tensor * sink_tail = dst->src[0];
                ...
                ggml_cuda_kvarn_materialize_kv_f16(
                        ...
                        (const float *) pending->data,
                        (uint16_t *) dst->data,
                        uint32_t(params.is_v),
                        uint32_t(params.n_sink),
                        uint32_t(params.n_records),
                        uint32_t(params.n_pending),
                        uint32_t(params.n_tail),
                        uint32_t(params.tail_start),
                        uint32_t(sink_tail->ne[1]),
                        uint32_t(params.head_dim),
                        uint32_t(params.group_size),
                        ...
                        size_t(body->nb[1]),
                        size_t(body->nb[2]),
                        size_t(scales->nb[1]/sizeof(float)),
                        size_t(scales->nb[2]/sizeof(float)),
                        size_t(pending->nb[1]/sizeof(float)),
                        size_t(pending->nb[2]/sizeof(float)),
                        size_t(dst->nb[1]/sizeof(uint16_t)),
                        size_t(dst->nb[2]/sizeof(uint16_t)),
                        ctx.stream());
            } break;
        case GGML_OP_KVARN_ATTN_MIXED:
            {
                ...
                const ggml_tensor * window      = dst->src[11] != nullptr && dst->src[11]->type == GGML_TYPE_I32 ? dst->src[11] : nullptr;
                const ggml_tensor * q_body      = dst->src[12] != nullptr && dst->src[12]->type == GGML_TYPE_F32 ? dst->src[12] : nullptr;
                const ggml_tensor * body_k_root = body_k;
                while (body_k_root != nullptr && body_k_root->view_src != nullptr) {
                    body_k_root = body_k_root->view_src;
                }
                const void * body_k_store_key = body_k_root != nullptr ? body_k_root->data : body_k->data;
                ...
                if (kq_mask_type != 0u &&
                        !ggml_cuda_kvarn_env_flag("LLAMA_KVARN_DISABLE_INTERNAL_CAUSAL_MASK")) {
                    kq_mask_type = 3u;
                }
                if (kq_mask_type != 0u &&
                        params.n_records > 0 &&
                        ggml_cuda_kvarn_env_flag("LLAMA_KVARN_ATTN_FORCE_COMPACT_CAUSAL_MASK")) {
                    kq_mask_type = 3u;
                    kq_mask = nullptr;
                }
                ...
                const bool use_raw_body_k = ggml_cuda_kvarn_env_flag("LLAMA_KVARN_DEBUG_RAW_BODY_K");
                const bool use_raw_body_v = ggml_cuda_kvarn_env_flag("LLAMA_KVARN_DEBUG_RAW_BODY_V");
                const bool raw_body_scalar_qt =
                    ggml_cuda_kvarn_env_flag("LLAMA_KVARN_DEBUG_RAW_BODY_SCALAR_QT") &&
                    (use_raw_body_k || use_raw_body_v);
                ...
                if (ggml_cuda_kvarn_attn_trace_enabled() && ggml_cuda_kvarn_attn_trace_claim()) {
                    ...
                    const bool split_runtime = use_scratch_ref || forced_split ||
                        mixed_frame || ((use_raw_body_k || use_raw_body_v) && !raw_body_scalar_qt);
                    ...
                    const char * mode = raw_body_scalar_qt ? "raw-body-scalar-qt" :
                        (use_scratch_ref ? "scratch-ref" :
                        (split_runtime ? "split" :
                    ...
                    std::fprintf(stderr,
                            ...
                            " scale=%.9g logit_softcap=%.9g mask_type=%u frame_flags=%d fused_paper_frame=%d v_layout=%d"
                            " q_body=%d raw_body_k=%d raw_body_v=%d\n",
                            ...
                            params.key_bits, params.value_bits, double(params.scale), double(params.logit_softcap), kq_mask_type,
                            params.frame_flags,
                            (params.frame_flags & GGML_KVARN_ATTN_FRAME_FUSED_PAPER_FULL) != 0 ? 1 : 0,
                            params.v_layout,
                            mixed_frame ? 1 : 0, use_raw_body_k ? 1 : 0, use_raw_body_v ? 1 : 0);
                    std::fprintf(stderr,
                ...
                std::ostringstream timing_label;
                timing_label << "attn-mixed"
                    << " head_dim=" << params.head_dim
                    << " n_queries=" << dst->ne[2]
                    << " n_head=" << dst->ne[1]
                    << " n_head_kv=" << sink_tail_k->ne[1]
                    << " records=" << params.n_records
                    << " pending=" << params.n_pending
                    << " sink=" << params.n_sink
                    << " tail=" << params.n_tail;
                ggml_cuda_kvarn_timing_scope timing(ctx.stream(), timing_label.str());
                ...
                if (use_scratch_ref && params.n_records > 0) {
                    ...
                    ggml_cuda_kvarn_attn_mixed_f16_batch_scratch(
                            (const float *) q->data,
                            q_body != nullptr ? (const float *) q_body->data : nullptr,
                            ...
                            q->nb[1]/sizeof(float),
                            q->nb[2]/sizeof(float),
                            q_body != nullptr ? q_body->nb[1]/sizeof(float) : q->nb[1]/sizeof(float),
                            q_body != nullptr ? q_body->nb[2]/sizeof(float) : q->nb[2]/sizeof(float),
                    ...
                    ggml_cuda_kvarn_attn_mixed_f16_batch(
                            ...
                            params.scale,
                            ctx.stream(),
                            window != nullptr ? (const int32_t *) window->data : nullptr,
                            scratch_nelems,
                            dst->src[3]->ne[1],
                            body_k_store_key,
                            uint32_t(params.frame_flags),
                            params.logit_softcap,
                            turbo_v_mode);
                }
                ggml_cuda_kvarn_maybe_dump_boundary(
                        q, q_body, sink_tail_k, sink_tail_v, body_k, body_v, scales_k, scales_v,
                        pending_k, pending_v, kq_mask, dst,
                        params.n_sink, params.n_records, params.n_pending, params.n_tail,
...
static bool ggml_cuda_graph_check_compability(ggml_cgraph * cgraph) {
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        if (node->op == GGML_OP_KVARN_ATTN_MIXED &&
                (node->src[11] == nullptr || node->src[11]->type != GGML_TYPE_I32)) {
        ...
        case GGML_OP_KVARN_STORE_BODY:
            {
                ...
                const int64_t scratch_floats = scratch_base +
                    ((params.is_v && ggml_cuda_kvarn_v_layout_is_residual(params.v_layout)) ? n : 0);
        ...
        case GGML_OP_KVARN_STORE_KV_BODY:
            {
                ...
                if (params.src_layout == 1) {
                    ...
                    int64_t scratch_floats = 2*tile_floats + pipeline_scratch_floats;
                    if (ggml_cuda_kvarn_v_layout_is_residual(params.v_layout)) {
                        const int64_t tiles = int64_t(params.n_heads)*params.n_record_batch;
                        scratch_floats = tiles*(3*tile_floats + 2*(params.head_dim + params.group_size + 2));
                    }
                    ...
                    return op->src[5]->ne[0] >= (int64_t) ((size_t(params.head_dim)*params.group_size*params.key_bits + 7)/8) &&
                           ggml_cuda_kvarn_v_body_shape_ok(op->src[6]->ne[0], v_body_bytes_direct, params.v_layout) &&
                ...
                const int64_t batch_scratch_floats = 2*n + pipeline_scratch_floats +
                    (ggml_cuda_kvarn_v_layout_is_residual(params.v_layout) ? n : 0);
                ...
                if (params.n_record_batch > 0) {
                    return params.n_record_batch == 1 &&
                           ...
                           ggml_cuda_kvarn_v_body_shape_ok(op->src[6]->ne[0], v_body_bytes, params.v_layout) &&
        ...
        case GGML_OP_KVARN_MATERIALIZE_KV:
            {
                ...
                if ((params.is_v != 0 && params.is_v != 1) ||
                        params.n_sink < 0 || params.n_records < 0 ||
                        params.n_pending < 0 || params.n_tail < 0 ||
                        params.tail_start < 0 ||
                ...
                const int64_t n_body = int64_t(params.n_records)*params.group_size;
                const int64_t n_kv = int64_t(params.n_sink) + n_body + params.n_pending + params.n_tail;
                const int64_t body_bytes = params.is_v ?
                    ggml_cuda_kvarn_v_body_bytes(params.head_dim, params.group_size, params.bits, params.v_layout) :
                    (int64_t(params.head_dim)*params.group_size*params.bits + 7)/8;
                if (body_bytes < 0) {
                    return false;
                }
                const int64_t scale_floats = params.is_v ?
                    int64_t(params.head_dim) + 2*int64_t(params.group_size) :
                    2*int64_t(params.head_dim) + int64_t(params.group_size);

                return op->ne[0] == params.head_dim &&
                       op->ne[1] == op->src[0]->ne[1] &&
                       op->ne[2] == n_kv &&
                       op->src[0]->ne[0] == params.head_dim &&
                       op->src[0]->ne[2] >= params.n_sink + params.n_tail &&
                       ggml_cuda_kvarn_v_body_shape_ok(op->src[1]->ne[0], body_bytes, params.is_v ? params.v_layout : GGML_CUDA_KVARN_V_LAYOUT_LEGACY) &&
                       op->src[1]->ne[1] >= params.n_records &&
                       op->src[1]->ne[2] == op->src[0]->ne[1] &&
                       op->src[2]->ne[0] >= scale_floats &&
                       op->src[2]->ne[1] >= params.n_records &&
                       op->src[2]->ne[2] == op->src[0]->ne[1] &&
                       op->src[3]->ne[0] == params.head_dim &&
                       op->src[3]->ne[1] == op->src[0]->ne[1] &&
                       op->src[3]->ne[2] >= params.n_pending;
            } break;
        case GGML_OP_KVARN_ATTN_MIXED:
            {
                struct kvarn_attn_params {
                ...
                if (op->type != GGML_TYPE_F32 || op->src[0]->type != GGML_TYPE_F32 ||
                        ...
                        (op->src[11] != nullptr &&
                         !(op->src[11]->type == GGML_TYPE_I32 && ggml_nelements(op->src[11]) >= 5)) ||
                        (op->src[12] != nullptr &&
                         !(op->src[12]->type == GGML_TYPE_F32 &&
                           op->src[12]->ne[0] == op->ne[0] &&
                           op->src[12]->ne[1] == op->ne[1] &&
                           op->src[12]->ne[2] == op->ne[2]))) {
                    return false;
                }
                if (params.n_sink < 0 || params.n_records < 0 || params.n_pending < 0 || params.n_tail < 0 ||
                        ...
                        params.key_bits <= 0 || params.key_bits > 8 ||
                        params.value_bits <= 0 || params.value_bits > 8 ||
                        (params.v_layout != GGML_CUDA_KVARN_V_LAYOUT_LEGACY &&
                         params.v_layout != GGML_CUDA_KVARN_V_LAYOUT_TURBO_CANONICAL &&
                         params.v_layout != GGML_CUDA_KVARN_V_LAYOUT_LEGACY_R1 &&
                         params.v_layout != GGML_CUDA_KVARN_V_LAYOUT_LEGACY_R2 &&
                         params.v_layout != GGML_CUDA_KVARN_V_LAYOUT_LEGACY_R3 &&
                         params.v_layout != GGML_CUDA_KVARN_V_LAYOUT_SPARSE_D512) ||
                        (params.frame_flags & ~GGML_KVARN_ATTN_FRAME_FUSED_PAPER_FULL) != 0) {
                    return false;
                }
                ...
                const bool body_mirror_requested =
                    ggml_cuda_kvarn_env_flag("LLAMA_KVARN_ATTN_REF_SCRATCH") ||
                    ggml_cuda_kvarn_env_flag("LLAMA_KVARN_ENABLE_F32_DEQUANT_CACHE") ||
                    ggml_cuda_kvarn_env_flag("LLAMA_KVARN_ATTN_ENABLE_BODY_F32_MIRROR");
                if (params.n_records > 0 && body_mirror_requested) {
                ...
                if (ggml_cuda_kvarn_v_layout_is_residual(params.v_layout) &&
                        (op->src[4]->ne[0] != v_body_bytes ||
                         op->src[4]->nb[0] != 1 ||
                         op->src[4]->nb[1] < size_t(v_body_bytes) ||
                         op->src[4]->nb[2] < op->src[4]->nb[1]*size_t(op->src[4]->ne[1]) ||
                         params.group_size != 128 || params.key_bits != 8 || params.value_bits != 2 ||
                         (params.head_dim != 256 && params.head_dim != 512))) {
                    return false;
                }
