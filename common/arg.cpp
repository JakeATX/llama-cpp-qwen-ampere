...
static std::vector<int> parse_atx_int_set(const std::string & value) {
    std::set<int> result;
    for (const auto & item : string_split<std::string>(value, ',')) {
        ...
        if (dash == std::string::npos) {
            ...
            if (v < 0) {
                throw std::invalid_argument("negative ids are not valid");
            }
            result.insert(v);
            continue;
        }
...
static std::vector<int> parse_atx_layer_expert_layers(const std::string & value) {
    std::set<int> layers;
    for (const auto & item : string_split<std::string>(value, ',')) {
        if (item.empty()) {
            continue;
        ...
        if (colon == std::string::npos) {
            throw std::invalid_argument("expected LAYER:EXPERT");
        }
        const int layer  = std::stoi(item.substr(0, colon));
        const int expert = std::stoi(item.substr(colon + 1));
        if (layer < 0 || expert < 0) {
            throw std::invalid_argument("negative ids are not valid");
        }
        layers.insert(layer);
...
static ggml_backend_buffer_type_t atx_first_gpu_buffer_type() {
    ggml_backend_load_all();
    for (size_t i = 0; i < ggml_backend_dev_count(); ++i) {
    ...
    throw std::invalid_argument("ATX MoE GPU residency requested, but no GPU buffer type is available");
}

static void atx_append_moe_layer_overrides(
        std::vector<llama_model_tensor_buft_override> & dst,
        const std::vector<int> & layers,
        ggml_backend_buffer_type_t buft) {
    static std::list<std::string> patterns;
    for (const int layer : layers) {
        patterns.push_back(llm_ffn_exps_block_regex(layer));
        dst.push_back({patterns.back().c_str(), buft});
    }
}

static void atx_prepend_exact_moe_layer_residency(
        std::vector<llama_model_tensor_buft_override> & overrides,
        const std::vector<int> & gpu_layers) {
    std::vector<llama_model_tensor_buft_override> policy;
    atx_append_moe_layer_overrides(policy, gpu_layers, atx_first_gpu_buffer_type());
    policy.push_back(llm_ffn_exps_cpu_override());
    overrides.insert(overrides.begin(), policy.begin(), policy.end());
}

static void atx_set_env(const char * key, const std::string & value) {
#if defined(_WIN32)
...
static void atx_enable_expert_residency(common_params & params) {
    // Exact expert residency depends on the existing CPU-MoE scheduler path,
    // which only copies selected experts from host-visible buffers. CPU_REPACK
    // is CPU-backed but not exposed as host-visible to ggml-backend.cpp.
    params.no_extra_bufts = true;
    params.tensor_buft_overrides.push_back(llm_ffn_exps_cpu_override());
}

static std::string atx_join_ints(const std::vector<int> & values) {
...
static std::vector<int> atx_parse_expert_set(const std::string & value) {
    std::vector<int> experts = parse_atx_int_set(value);
    for (const int expert : experts) {
        if (expert > 255) {
            throw std::invalid_argument("expert id must be in range 0..255");
        }
    }
    return experts;
}

static std::string atx_normalize_expert_set(const std::string & value) {
    return atx_join_ints(atx_parse_expert_set(value));
}

static std::string atx_normalize_layer_experts(const std::string & value) {
    if (value.empty()) {
        throw std::invalid_argument("empty layer-expert policy");
    }
    ...
    while (start < value.size()) {
        ...
        if (item.empty()) {
            throw std::invalid_argument("empty layer-expert entry");
        }

        const size_t colon = item.find(':');
        if (colon == std::string::npos || item.find(':', colon + 1) != std::string::npos) {
            throw std::invalid_argument("layer-expert entries must use L:E format");
        }

        const int layer = std::stoi(item.substr(0, colon));
        std::vector<int> experts = atx_parse_expert_set(item.substr(colon + 1));
        if (layer < 0) {
            throw std::invalid_argument("layer id must be non-negative");
        }
        for (const int expert : experts) {
            cells.insert({layer, expert});
        }
...
static void atx_apply_residency_policy_file(common_params & params, const std::string & path) {
    ...
    auto read_experts = [&](const char * key) -> bool {
        ...
        if (policy.at(key).is_string()) {
            atx_set_env("ATX_MOE_KEEP_EXPERTS", atx_normalize_expert_set(policy.at(key).get<std::string>()));
            return true;
        }
        if (!policy.at(key).is_array()) {
            throw std::invalid_argument(string_format("policy key '%s' must be a string or array", key));
        }
        for (const auto & item : policy.at(key)) {
            const int expert = item.get<int>();
            if (expert < 0 || expert > 255) {
                throw std::invalid_argument("policy expert id must be in range 0..255");
            }
            experts.push_back(expert);
        }
        std::sort(experts.begin(), experts.end());
        experts.erase(std::unique(experts.begin(), experts.end()), experts.end());
        atx_set_env("ATX_MOE_KEEP_EXPERTS", atx_join_ints(experts));
        return true;
    };

    bool has_expert_policy =
        read_experts("keep_experts") ||
        read_experts("experts") ||
        read_experts("moe_keep_experts");

    auto read_layer_experts = [&](const char * key) -> bool {
        if (!policy.contains(key)) {
            return false;
        }
        ...
        const auto & val = policy.at(key);
        if (val.is_string()) {
            atx_set_env("ATX_MOE_KEEP_LAYER_EXPERTS", atx_normalize_layer_experts(val.get<std::string>()));
            return true;
        }
        if (val.is_array()) {
            for (const auto & item : val) {
                if (item.is_string()) {
                    cells.push_back(item.get<std::string>());
                } else {
                    cells.push_back(string_format("%d:%d", item.at("layer").get<int>(), item.at("expert").get<int>()));
                }
            }
            ...
            for (auto it = val.begin(); it != val.end(); ++it) {
                ...
                if (!it.value().is_array()) {
                    throw std::invalid_argument(string_format("policy key '%s' object values must be arrays", key));
                }
                for (const auto & expert : it.value()) {
                    cells.push_back(string_format("%d:%d", layer, expert.get<int>()));
                }
            }
        } else {
            throw std::invalid_argument(string_format("policy key '%s' must be a string, array, or object", key));
        }
        ...
        atx_set_env("ATX_MOE_KEEP_LAYER_EXPERTS", atx_normalize_layer_experts(out.str()));
        return true;
    };

    has_expert_policy =
        read_layer_experts("keep_layer_experts") ||
        read_layer_experts("layer_experts") ||
...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--moe-cpu-layers"}, "L0,L1-L2,...",
        "ATX: keep MoE expert tensors for the listed layers in CPU/RAM",
        [](common_params & params, const std::string & value) {
            atx_append_moe_layer_overrides(params.tensor_buft_overrides, parse_atx_int_set(value), ggml_backend_cpu_buffer_type());
        }
    ).set_env("LLAMA_ARG_MOE_CPU_LAYERS"));
    add_opt(common_arg(
        {"--moe-gpu-layers"}, "L0,L1-L2,...",
        "ATX: keep MoE expert tensors for the listed layers in the first GPU/VRAM buffer",
        [](common_params & params, const std::string & value) {
            atx_append_moe_layer_overrides(params.tensor_buft_overrides, parse_atx_int_set(value), atx_first_gpu_buffer_type());
        }
    ).set_env("LLAMA_ARG_MOE_GPU_LAYERS"));
    add_opt(common_arg(
        {"--moe-keep-layers"}, "L0,L1-L2,...",
        "ATX: exact MoE layer residency; listed MoE layers stay GPU/VRAM-resident and all other MoE expert tensors go to CPU/RAM",
        [](common_params & params, const std::string & value) {
            atx_prepend_exact_moe_layer_residency(params.tensor_buft_overrides, parse_atx_int_set(value));
        }
    ).set_env("LLAMA_ARG_MOE_KEEP_LAYERS"));
    add_opt(common_arg(
        {"--moe-keep-layer-experts"}, "L:E,L:E,...",
        "ATX: keep listed MoE layer-expert cells in the expert residency cache and source the remaining routed experts from CPU/RAM",
        [](common_params & params, const std::string & value) {
            atx_set_env("ATX_MOE_KEEP_LAYER_EXPERTS", atx_normalize_layer_experts(value));
            atx_enable_expert_residency(params);
        }
    ).set_env("LLAMA_ARG_MOE_KEEP_LAYER_EXPERTS"));
    add_opt(common_arg(
        {"--moe-promote-layer-experts"}, "L:E,L:E,...",
        "ATX: packed-GGUF compatibility helper; promotes listed layer-expert cells to exact whole-layer GPU/VRAM residency",
        [](common_params & params, const std::string & value) {
            atx_prepend_exact_moe_layer_residency(params.tensor_buft_overrides, parse_atx_layer_expert_layers(value));
        }
    ).set_env("LLAMA_ARG_MOE_PROMOTE_LAYER_EXPERTS"));
    add_opt(common_arg(
        {"--moe-keep-experts"}, "E0,E1-E2,...",
        "ATX: keep listed MoE expert IDs in the expert residency cache across packed MoE layers and source other routed experts from CPU/RAM",
        [](common_params & params, const std::string & value) {
            atx_set_env("ATX_MOE_KEEP_EXPERTS", atx_normalize_expert_set(value));
            atx_enable_expert_residency(params);
        }
