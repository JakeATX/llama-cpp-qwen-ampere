...
void llama_model_qwen35moe_mtp::load_arch_hparams(llama_model_loader & ml) {
    ...
    GGML_ASSERT(hparams.n_layer_nextn > 0   && "QWEN35MOE_MTP requires n_layer_nextn > 0");
...
llama_model_qwen35moe_mtp::graph::graph(const llama_model & model, const llm_graph_params & params)
    ...
    GGML_ASSERT(hparams.n_layer_nextn > 0 && "QWEN35MOE_MTP requires n_layer_nextn > 0");
    GGML_ASSERT(hparams.n_layer_nextn == 1 && "QWEN35MOE_MTP currently only supports a single MTP block");

    const int64_t n_embd_head = hparams.n_embd_head_v();
    GGML_ASSERT(n_embd_head == hparams.n_embd_head_k());

    const int il = (int) hparams.n_layer();
